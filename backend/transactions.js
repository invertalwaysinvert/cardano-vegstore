...
function getFees(req, res) {
  ...
  if (db.get(email)) {
      var walletId = db.get(email).id
      var postData = req.body;
      axios.post(`${process.env.WALLET_SERVER}/v2/wallets/${walletId}/payment-fees`, postData, { headers: {
          'Content-Type': 'application/json'
        }}).then(function (response) {
          res.status(200).send(response.data);
        }).catch(function (error) {
...
function forgetTransaction(req, res) {
  var email = req.query.email
  var transactionId = req.query.transactionId
    if (email == undefined || email == '' || transactionId == undefined || transactionId == '') {
        res.status(400).send();
        return
    }
    if (db.get(email)) {
      var walletId = db.get(email).id
      axios.delete(`${process.env.WALLET_SERVER}/v2/wallets/${walletId}/transactions/${transactionId}`, { headers: {
          'Content-Type': 'application/json'
        }}).then(function (response) {
          res.status(200).send(response.data);
        }).catch(function (error) {
